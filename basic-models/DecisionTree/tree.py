...
class DTreeRegressor:
    ...
    '''Split a node into left and right children'''
    '''Aim: Find the best split point with lowest MSE'''
    def __node_split(self, data_ids):
        '''
            data_ids: contains the id's of the data points present in the node
        '''
        ...
        for fid in range(self.x_train.shape[1]):
            ...
            '''
               len(candidate_split_points) > 1:
                   Calculate the mse for all candidates, and find the candidate with the lowest mse.
               len(candidate_split_points) < 1:
                   Skip the for-loop. feature_data either has only one data point or all have the same value. 
                   No need to split.
            '''
            ...
            for c in candidate_split_points:
                ...
                l_mse=self.y_train[left].var()
                r_mse=self.y_train[right].var()

                # Calculate IG
                overall_mse=l_mse * (left.shape[0]/n) + r_mse * (right.shape[0]/n)

                # find where the MSE is smallest
                if overall_mse < min_mse:
                    min_mse=overall_mse
                    b_fid=fid # best feature id
                    b_point=c # best split point
                    l_node=left # data ids in left node
    ...
    # Change the data ids in the leaf node to average value
    def __update_leaf(self, node):
        if node['is_leaf']==False:
            for key, value in node.items():
                if key == 'left_child' or key == 'right_child':
            ...
            return 1, np.mean(self.y_train[node['ids']]) # first 1 means this is a leaf node
